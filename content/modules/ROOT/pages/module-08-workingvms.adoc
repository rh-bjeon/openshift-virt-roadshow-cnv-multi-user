...
. *winweb01-{user}* VM을 선택하고 `설정` 탭으로 이동합니다.
. 화면 중앙 하단에서 `메타데이터` 메뉴를 누릅니다. 
. 라벨 옆의 `편집` 을 누릅니다.

+
image::2025_spring/module-08-workingvms/12_Imported_VMs_UI.png[link=self, window=blank, width=100%]

. 아래의 레이블을 붙여넣고 `저장` 을 누릅니다.
+
...
----
+
image::2025_spring/module-08-workingvms/12_Imported_VMs_UI2.png[link=self, window=blank, width=100%]

. 아래 스크린샷처럼 *env=webapp* 레이블이 설정된 것이 확인됩니다.

image::2025_spring/module-08-workingvms/12_Imported_VMs_UI3.png[link=self, window=blank, width=100%]
