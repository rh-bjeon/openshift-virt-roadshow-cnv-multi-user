...
----
+
IMPORTANT: 서비스(Service) YAML에서 가상 머신의 네임스페이스가 올바른지 확인하세요. (예를 들어 미리 가져오기가 완료되어 있던 가상 머신을 사용 중이시라면 네임스페이스는 위에 적힌 **vmexamples-{user}**가 아니라 **vmimported-{user}**로 수정하셔야 합니다.)
+
...
. 왼쪽 탐색 메뉴에서 *네트워킹* 메뉴 아래쪽, *경로* 옵션을 클릭합니다. 화면 중앙에 있는 *경로 생성* 버튼을 클릭합니다.
+
...
. 다시 한번, *네트워킹* → **서비스**로 이동하여 **서비스 생성**을 누릅니다. YAML 파일을 다음 정의로 바꿉니다.
+
[source,yaml,role=execute,subs="attributes"]
----
apiVersion: v1
