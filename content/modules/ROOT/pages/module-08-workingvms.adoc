...
IMPORTANT: `기존 가상 머신 마이그레이션` 모듈을 아직 완료하지 않았다면 먼저 해당 모듈을 완료하는 것이 좋습니다. 모듈을 완료하지 않았거나 마이그레이션 프로세스가 아직 진행 중인 경우, *vmimported-{user}* 프로젝트에 있는 미리 준비된 가상 머신을 사용할 수 있습니다. 미리 가져온 가상 머신을 사용하는 경우, 아래 예제에서 *vmexamples-{user}* 네임스페이스를 모두 **vmimported-{user}**로 바꿔야 합니다.
+
